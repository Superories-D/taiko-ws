...
`ALLOWED_ORIGINS` is a comma-separated exact origin allowlist. Do not use a wildcard. The health endpoint intentionally exposes only service state, aggregate connection count, and capacity. Any compatible third-party node must return `status: "ok"`, an integer `connections`, and a boolean `accepting_connections`; otherwise the main site will not route players to it. The service enforces `MAX_CONNECTIONS` at both handshake and connection time, limits messages to 64 KiB, bounds queueing, rejects per-connection floods above `MAX_MESSAGES_PER_SECOND`, limits invitation guessing, and isolates malformed client messages so one client cannot crash the process.
